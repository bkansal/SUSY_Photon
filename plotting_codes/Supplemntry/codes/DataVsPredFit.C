...
vector<int> col={kBlack,kCyan,kOrange,kTeal+9,kCyan-1,kBlue,kMagenta+3,kBlue,kRed};
TString name;
...
void DataVsPredFit(TString sample="general",TString iFname="rootoutput/SR_prefitPredictions_Run2_v4.root", TString catName="CR"){
  ...
  if(sample.Contains("T5bbbbZg"))
    {
      sampleName = "../rootoutput/nphotons/T5bbbbZg_2250_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5bbbbZg_2250_1600_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5bbbbZg_2250_2100_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("T5qqqqHg"))
    {
      sampleName = "../rootoutput/nphotons/T5qqqqHg_2250_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5qqqqHg_2250_1600_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5qqqqHg_2250_2150_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("T5ttttZg"))
    {
      sampleName = "../rootoutput/nphotons/T5ttttZg_2200_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5ttttZg_2200_1000_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T5ttttZg_2200_1975_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("T6ttZg"))
    {
      sampleName = "../rootoutput/nphotons/T6ttZg_1300_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T6ttZg_1300_600_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/T6ttZg_1300_1200_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("TChiWg"))
    {
      sampleName = "../rootoutput/nphotons/TChiWG_0_600_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiWg_0_1000_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiWg_0_1200_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("TChiNg"))
    {
      sampleName = "../rootoutput/nphotons/TChiNG_0_600_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiNG_0_1000_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiNG_0_1200_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }
  else if(sample.Contains("TChiNG_nn"))
    {
      sampleName = "../rootoutput/nphotons/TChiNGnn_0_500_FastSim_v18.root"; fSig[0] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiNGnn_0_600_FastSim_v18.root"; fSig[1] = new TFile(sampleName);
      sampleName = "../rootoutput/nphotons/TChiNGnn_0_700_FastSim_v18.root"; fSig[2] = new TFile(sampleName);
    }

  else{
    sampleName = "../rootoutput/nphotons/T5bbbbZg_2200_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);        
    sampleName = "../rootoutput/nphotons/T5bbbbZg_2200_2100_FastSim_v18.root"; fSig[1] = new TFile(sampleName);	
    sampleName = "../rootoutput/nphotons/TChiWG_0_1000_FastSim_v18.root"; fSig[2] = new TFile(sampleName); 
  }
  ...
  */
  ...
  p_top->SetRightMargin(0.03);
  p_bot->SetTopMargin(0);
  p_bot->SetRightMargin(0.03);
  p_bot->SetBottomMargin(0.3);
  ...
  // }
  h_sum->Draw("e2 same");
  hs_hist->GetXaxis()->SetTickLength(0);
  TF1 *f2_=new TF1("f2_","x",1.5,46.5);
  TGaxis *A2 = new TGaxis(1,0.15,46,0.15,"f2_",45,"US");
  A2->SetTickSize(0.02);
  A2->Draw("B");
 ...
 for(int k=1; k<46;k++)
    if(h_data->GetBinContent(k)==0) h_data->SetBinError(k,1.84);


 h_data->Draw("x0e0 same");
  ...
  //  for(int k=1; k<46;k++)
  //  if(h_numr->GetBinContent(k)==0) h_data->SetBinError(k,1.8/h_sum->GetBinContent(k));
  ...
  p_bot->cd();
  TString temp2;
  for(double j=1;j<=h_numr->GetNbinsX();j++){
    temp2 = to_string(j);
    if((j)==5){temp2=to_string(5); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==10){temp2=to_string(10); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==15){temp2=to_string(15); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==20){temp2=to_string(20); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==25){temp2=to_string(25); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==30){temp2=to_string(30); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==35){temp2=to_string(35); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==40){temp2=to_string(40); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}
    else if((j)==45){temp2=to_string(45); h_numr->GetXaxis()->SetBinLabel(j+0.5,temp2);}

  }
  h_numr->GetXaxis()->LabelsOption("h");
  h_numr->GetXaxis()->SetLabelSize(0.20);
  h_numr->GetXaxis()->SetLabelOffset(0.01);
  h_numr->GetXaxis()->SetTickLength(0);
  TF1 *f1_=new TF1("f1_","x",1.5,46.5);
  TGaxis *A1 = new TGaxis(1,0.01,46,0.01,"f1_",45,"US");
  A1->SetTickSize(0.05);
  A1->Draw("B");
  ...
  textOnTop2.DrawLatexNDC(0.16,0.915,"#it{#bf{Supplementary}}");
  sprintf(name2,"#bf{%0.0f fb^{-1} (13 TeV)}",intLumi);
  intLumiE.DrawLatexNDC(0.818,0.92,name2);
  if(saveCanvas){
    //    c_cA->SetCanvasSize(1600,900);
    if(iFname.Contains("PostFit")) {name=catName+year1+"_postFit.pdf";c_cA->SaveAs(name);}
    else if(iFname.Contains("prefit")) {name="png/"+sample+"_prefit.png"; c_cA->SaveAs(name);    c_cA->SetCanvasSize(1600,900); name="pdf/"+sample+"_prefit.pdf"; c_cA->SaveAs(name);}
  ...
  if(printTable){
    ...
    for(int i=0;i<9;i++)
      {
	if(i==0){
	  cout<<"\\multicolumn{8}{c}{"<<Nbj[i]<<"} \\ "<<endl;
	  cout<<"\\hline"<<endl;
	  for(int j=0 ; j<7 ; j++)
	    {
	      k=k+1;
	      ...
	      cout<<k<<" & "<<METbin[j]<<" & "<<ll->GetBinContent(k)<<" $\\pm$ "<<ll->GetBinError(k)<<" & "<<fr->GetBinContent(k)<<" $\\pm$ "<<fr->GetBinError(k)<<" & "<<zin->GetBinContent(k)<<" $\\pm$ "<<zin->GetBinError(k)<<" & "<<multi->GetBinContent(k)<<" $\\pm$ "<<multi->GetBinError(k)<<" & "<<total->GetBinContent(k)<<" $\\pm$ "<<total->GetBinError(k)<<" & "<<hd->GetBinContent(k)<<" \\ "<<endl;
	    }
	}
	else if(i==1){
	  cout<<"\\multicolumn{8}{c}{"<<Nbj[i]<<"} \\ "<<endl;
	  ...
	  for(int j=0 ; j<6 ; j++)
	    {
	      k=k+1;
	      pull2 = (hd->GetBinContent(k) - total->GetBinContent(k))/sqrt(total->GetBinError(k)*total->GetBinError(k) + hd->GetBinError(k)*hd->GetBinError(k));
	      cout<<k<<" & "<<METbin2[j]<<" & "<<ll->GetBinContent(k)<<" $\\pm$ "<<ll->GetBinError(k)<<" & "<<fr->GetBinContent(k)<<" $\\pm$ "<<fr->GetBinError(k)<<" & "<<zin->GetBinContent(k)<<" $\\pm$ "<<zin->GetBinError(k)<<" & "<<multi->GetBinContent(k)<<" $\\pm$ "<<multi->GetBinError(k)<<" & "<<total->GetBinContent(k)<<" $\\pm$ "<<total->GetBinError(k)<<" & "<<hd->GetBinContent(k)<<" \\ "<<endl;
	    }
	}
	else if(i<6){
	  cout<<"\\multicolumn{8}{c}{"<<Nbj[i]<<"} \\ "<<endl;
	  ...
	  for(int j=0 ; j<5 ; j++)
	    {
	      k=k+1;
              pull2 = (hd->GetBinContent(k) - total->GetBinContent(k))/sqrt(total->GetBinError(k)*total->GetBinError(k) + hd->GetBinError(k)*hd->GetBinError(k));
	      cout<<k<<" & "<<METbin3[j]<<" & "<<ll->GetBinContent(k)<<" $\\pm$ "<<ll->GetBinError(k)<<" & "<<fr->GetBinContent(k)<<" $\\pm$ "<<fr->GetBinError(k)<<" & "<<zin->GetBinContent(k)<<" $\\pm$ "<<zin->GetBinError(k)<<" & "<<multi->GetBinContent(k)<<" $\\pm$ "<<multi->GetBinError(k)<<" & "<<total->GetBinContent(k)<<" $\\pm$ "<<total->GetBinError(k)<<" & "<<hd->GetBinContent(k)<<" \\ "<<endl;
