void plotpredsys_Zin()
{
  gStyle->SetOptStat(0);
  TLatex textOnTop,intLumiE,textOnTop2;

  TString path="rootoutput/Zin_purity/withphicut_zmass60to120/";
  TString path1="../Z_invisible/";
  TString filename  = path+"ZJZGTo2Nu_CR_v18.root";
  TString filename1  = path1+"Zinvi_highdphi.root";
  TFile *f = new TFile(filename);
  TFile *f1 = new TFile(filename1);
  TFile *f_syserr= new TFile("factors/Zinvi_EWKcorr_v2.root");
  //  TFile *f_syserr= new TFile("../Z_invisible/factors/Zinvi_sys.root");
  ...
  TH1D *cr = (TH1D*)f->Get("AllSBins_v6_CD_EW_50bin_elec0");
  //  pred->Print("all");
  // cr->Print("all");
  ...
  double ymin=0 , ymax=1.99, ymin_=0.0012 , ymax_=100000, yset_=2000;
  c1->SetLogy();
  syserr->Print("all");    
  for(int i=1; i<46; i++)
    {
      //      predsys->SetBinError(i+1,pred->GetBinContent(i+1)*pow(pow(syserr->GetBinError(i+1)/pred->GetBinContent(i+1),2),0.5));
      predsys->SetBinError(i+1,syserr->GetBinError(i+1));
  ...
  cr->SetLineWidth(2);
  pred->SetLineColor(kOrange);
  pred->SetFillColor(kOrange);
  ...
  c1->SaveAs("png/Zin_prediction.png");
  c1->SaveAs("pdf/Zin_prediction.pdf");
