...
void Fakerate::EventLoop(const char *data,const char *inputFileList) {
  ...
  if(highdphi)
    {
      if(apply_pixelveto)  TF = TFile::Open("../factors/fake_rate_highdphi_pxveto.root","READ");
      if(!apply_pixelveto) TF = TFile::Open("../factors/fake_rate_highdphi.root","READ");
      tf=(TH2D*)TF->FindObjectAny("fr");
    } 

  else
    {
      TF = TFile::Open("../factors/fake_rate_lowdphi.root","READ");
      tf=(TH2D*)TF->FindObjectAny("fr");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
   ...
   if(nlep==1 && v_lep1.Pt()>100 && !(s_data.Contains("data")))
    ...
    h_selectBaselineYields_2->Fill("1 e or 1 #gamma",wt);
   ...
   if(bestEMObjIsEle)  
     ...
     {
       mt_pho=sqrt(2*bestEMObj.Pt()*MET*(1-cos(DeltaPhi(METPhi,bestEMObj.Phi()))));
       //if(mt_pho>100 ) continue;                                                                                                                                         
       if(mt_pho<=100 ) pho_reco++;
     }

   bool fakePhoton=false;

   int nGenEle2=0, nGenPho1=0 , nGenEle1=0;
   TLorentzVector genEle1, genPho1 ;
   vector<TLorentzVector> v_genEle2,v_genPho2,v_genEle1;
   ...
   if(!s_data.Contains("data"))
     ...
     for(int i=0 ; i < GenParticles->size(); i++)
       {
	 if((*GenParticles)[i].Pt()!=0 && abs((*GenParticles_PdgId)[i])==11)
	   {
	     nGenEle2++;
     ...
     cout<<"=========== "<<jentry<<" ================="<<endl;
     if(v_genEle2.size()==0) cout<<"No gen electron"<<endl;
     else if(v_genEle2.size()==1) cout<<"1 gen electron"<<endl; 	
     else if(v_genEle2.size()==2)cout<<"2 gen electron"<<endl;
     */   
     }
   if(!bestEMObjIsEle && bestEMobj)
    ...
    for(int i=0;i<Jets->size();i++){
      if( ((*Jets)[i].Pt() > 30.0) && (abs((*Jets)[i].Eta()) <= 2.4) ){
	//	q++;
	//	if((*Jets_ID)[i]) h2_mindr_jetmatchphoratio->Fill(minDR,(*Jets)[minDRindx].Pt()/bestEMObj.Pt(),wt);
   ...
   if(applybTagSFs && s_data.Contains("v17")){
    ...
    double ratio=-999, mindr=-999;	
    if(!bestEMObjIsEle && bestEMobj)
      {
	double xmin=0.79,xmax=1.21;
	if(!s_data.Contains("data")){
	  
	  if(v_genEle2.size()==1 && bestPhoton.DeltaR(v_genEle2[0])<0.2 && bestPhoton.Pt()/v_genEle2[0].Pt()>xmin && bestPhoton.Pt()/v_genEle2[0].Pt()<xmax)
            {                                                                                                                                                             
              fail_realphoton++;                                                                                                                                          
              fakePhoton=true;                                                                                                                                            
              h_pass_events->Fill(2,wt);       
	      ratio=bestPhoton.Pt()/v_genEle2[0].Pt();
	      mindr=bestPhoton.DeltaR(v_genEle2[0]);
	    }
          else if(v_genEle2.size()>1 && ((bestPhoton.DeltaR(v_genEle2[0])<0.2 && bestPhoton.Pt()/v_genEle2[0].Pt()>xmin && bestPhoton.Pt()/v_genEle2[0].Pt()<xmax) || (bestPhoton.DeltaR(v_genEle2[1])<0.2 && bestPhoton.Pt()/v_genEle2[1].Pt()>xmin && bestPhoton.Pt()/v_genEle2[1].Pt()<xmax)))
            {
              fail_realphoton++;
              fakePhoton=true;
              h_pass_events->Fill(2,wt);
	      if(bestPhoton.DeltaR(v_genEle2[0])<0.2 && bestPhoton.Pt()/v_genEle2[0].Pt()>xmin && bestPhoton.Pt()/v_genEle2[0].Pt()<xmax) ratio=bestPhoton.Pt()/v_genEle2[0].Pt(), mindr=bestPhoton.DeltaR(v_genEle2[0]);
	      else if(bestPhoton.DeltaR(v_genEle2[1])<0.2 && bestPhoton.Pt()/v_genEle2[1].Pt()>xmin && bestPhoton.Pt()/v_genEle2[1].Pt()<xmax) ratio=bestPhoton.Pt()/v_genEle2[1].Pt(), mindr=bestPhoton.DeltaR(v_genEle2[1]);
	      else cout<<"!!!!!! both gen electrons are matching with photon"<<endl;
	      //	      h2_mindr_lep_goodph_ph->Fill(mindr,ratio,wt);
            }
	      ...
	      if(v_genEle2.size()==1) ratio=bestPhoton.Pt()/v_genEle2[0].Pt(), mindr=bestPhoton.DeltaR(v_genEle2[0]);
	  ...
	  else if ((*Photons_electronFakes)[bestPhotonIndxAmongPhotons])
	    {
	      fail_realphoton++;
	      fakePhoton=true;
	      h_pass_events->Fill(2,wt);
	...
	//	if(v_genEle2.size()>0) h2_mindr_lep_goodph_ph->Fill(bestPhoton.DeltaR(v_genEle2[0]),bestPhoton.Pt()/v_genEle2[0].Pt(),wt);
	if(v_genEle2.size()>0) h2_mindr_lep_goodph_ph->Fill(mindr,ratio,wt);
	h_genephoratio->Fill(ratio,wt);

    //    else cout<<"no genElectrons found : "<<v_genEle2.size()<<endl;
      }
    //v_genEle2
	...
	if(bestEMObjIsEle )
	  {
	    ...
	    if(applybTagSFs && s_data.Contains("v17")){
	      ...
	      h2_PhoPtQMultJet->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
              h2_SBins_v6_CD_EW_50bin_elec1->Fill(sBin6_50bin,qmulti,wt*prob0);
	      h2_PhoPtQMultJet_v2->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
	      sBin7_50bin= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nbjets,nHadJets);
	      h2_PhoPtQMultJet_fr[sBin7_50bin]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
	      h_MET->Fill(MET,wt*prob0);
	      h_METPhi->Fill(METPhi,wt*prob0);
		...
		for(int si=0;si<ScaleWeights->size();si++)
		  {
		    h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*ScaleWeights)[si]);
		    h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*ScaleWeights)[si]);
		  }
		for(int si=0;si<PDFweights->size();si++)
		  {
		    h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*PDFweights)[si]);
		  }
	      }
	      else
		{
		  for(int si=0;si<ScaleWeights->size();si++)
		    {
		      h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
		      h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
		    }
		  for(int si=0;si<PDFweights->size();si++)
		    {
		      h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
		      h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
		    }
		}
	      ...
	      nbjets=1;
              ...
              h2_SBins_v6_CD_EW_50bin_elec1->Fill(sBin6_50bin,qmulti,wt*prob1);
	      h2_PhoPtQMultJet->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
              h2_PhoPtQMultJet_v2->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
	      sBin7_50bin= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nbjets,nHadJets);
              h2_PhoPtQMultJet_fr[sBin7_50bin]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
              h_MET->Fill(MET,wt*prob1);
              h_METPhi->Fill(METPhi,wt*prob1);
		...
		for(int si=0;si<ScaleWeights->size();si++)
                  {
                    h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*ScaleWeights)[si]);
                    h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*ScaleWeights)[si]);
                  }
		for(int si=0;si<PDFweights->size();si++)
                  {
                    h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*PDFweights)[si]);
                  }
              }
              else
		{
                  for(int si=0;si<ScaleWeights->size();si++)
                    {
                      h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                      h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                    }
                  for(int si=0;si<PDFweights->size();si++)
		    {
                      h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                      h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                    }
                }
		...
		for(int si=0;si<11;si++)
                  {
                    h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*ScaleWeights)[si]);
                    h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*ScaleWeights)[si]);
                  }
		for(int si=0;si<111;si++)
                  {
                    h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*PDFweights)[si]);
                  }
              }
              else
		{
                  for(int si=0;si<11;si++)
                    {
                      h2_PhoPtQMultJet_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                      h2_PhoPtQMultJet_v2_scale[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                    }
                  for(int si=0;si<111;si++)
		    {
                      h2_PhoPtQMultJet_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                      h2_PhoPtQMultJet_v2_pdf[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                    }
                }
	    ...
	    */
	    ...
	    if(EWselec && EWselec1) h_hadAk8Mass_EW->Fill(Ak8Mass,wt);	  
            else h_hadAk8Mass_SP->Fill(Ak8Mass,wt);
		...
		//h_jetphoratio->Fill((*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
		//		if(minDRindx>=0) h2_mindr_jetmatchphoratio->Fill(minDR,(*Jets)[minDRindx].Pt()/bestEMObj.Pt(),wt);

	      }
	    if(photonMatchingJetIndx>=0){
	      h2_BestPhotonPt_jetmatchphoratio->Fill(bestEMObj.Pt(),(*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
	      h_jetphoratio->Fill((*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
	      //              h2_mindr_jetmatchphoratio->Fill(
	      if(minDRindx>=0) h2_mindr_jetmatchphoratio->Fill(minDR,(*Jets)[minDRindx].Pt()/bestEMObj.Pt(),wt);
	    ...
	    //	    h2_QMultlleadJetPt->Fill(leadjet_Pt,leadjet_qmulti,wt);
            h2_QMultlleadJetPt->Fill(bestEMObj.Pt(),qmulti,wt);
		...
		//		if(parX>599.99 && parX<601) cout<<jentry<<" : photon pt = "<<parX<<" , qmulti = "<<parY<<" , fakerate = "<<fakerate<<endl;		
		wt2=fakerate*wt;
		vector<double> SF,SF_err; 
		//SF={1.93,1.84,1,1};
		SF_err={0.2,0.2,0.1,0.1};
		SF={1.97325,2.36475,1,1}; 
		//SF_err={0.039,0.118,0.1,0.1};
		if(applySF && !applysys)
		  {
		    if(nbjets==0) wt2=wt2*SF[0];
		    else wt2=wt2*SF[1];
		  }	
		if(applySF && applysys){
		  if(nbjets==0) wt2=wt2*(SF[0]+SF_err[0]);
		  else wt2=wt2*(SF[1]+SF_err[1]);
		}
		...
		*/
		...
		if(applybTagSFs && s_data.Contains("v17")){
		  ...
		  h_SBins_v6_CD_EW_50bin_elec1_closure->Fill(sBin6_50bin,wt2*prob0);
                  h2_SBins_v6_CD_EW_50bin_elec1_closure->Fill(sBin6_50bin,qmulti,wt2*prob0);

		  h2_PhoPtQMultJet_elec1_closure->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
		  h2_PhoPtQMultJet_v2_elec1_closure->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
		  ...
		  if(savePDFscaleUnc){
		    for(int si=0;si<ScaleWeights->size();si++)
		      {
			h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0*(*ScaleWeights)[si]);
			h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0*(*ScaleWeights)[si]);
		      }
		    for(int si=0;si<PDFweights->size();si++)
		      {
			h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0*(*PDFweights)[si]);
			h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0*(*PDFweights)[si]);
		      }
		  }
		  else
		    {
		      for(int si=0;si<ScaleWeights->size();si++)
			{
			  h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
			  h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
			}
		      for(int si=0;si<PDFweights->size();si++)
			{
			  h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
			  h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob0);
			}
		    }
		  ...
		  h_SBins_v6_CD_EW_50bin_elec1_closure->Fill(sBin6_50bin,wt2*prob1);
                  h2_SBins_v6_CD_EW_50bin_elec1_closure->Fill(sBin6_50bin,qmulti,wt2*prob1);

		  h2_PhoPtQMultJet_elec1_closure->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
		  h2_PhoPtQMultJet_v2_elec1_closure->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
		  ...
		  h_BTags_elec1_closure->Fill(nbjets,wt2*prob1);
                  ...
                  if(savePDFscaleUnc){
                    for(int si=0;si<ScaleWeights->size();si++)
                      {
			h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1*(*ScaleWeights)[si]);
                        h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1*(*ScaleWeights)[si]);
                      }
                    for(int si=0;si<PDFweights->size();si++)
                      {
                        h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1*(*PDFweights)[si]);
                        h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1*(*PDFweights)[si]);
                      }
                  }
                  else
                    {
                      for(int si=0;si<ScaleWeights->size();si++)
                        {
                          h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
                          h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
                        }
                      for(int si=0;si<PDFweights->size();si++)
                        {
                          h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
                          h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*prob1);
                        }
                    }
		  ...
		  h_BTags_elec1_closure->Fill(nbjets,wt2);
                  if(savePDFscaleUnc){
                    for(int si=0;si<11;si++)
                      {
			h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*(*ScaleWeights)[si]);
                        h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*(*ScaleWeights)[si]);
                      }
                    for(int si=0;si<111;si++)
                      {
                        h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*(*PDFweights)[si]);
                        h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2*(*PDFweights)[si]);
                      }
                  }
                  else
                    {
                      for(int si=0;si<11;si++)
                        {
                          h2_PhoPtQMultJet_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2);
                          h2_PhoPtQMultJet_v2_scale_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2);
                        }
                      for(int si=0;si<111;si++)
                        {
                          h2_PhoPtQMultJet_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2);
                          h2_PhoPtQMultJet_v2_pdf_elec1_closure[si]->Fill(bestEMObj.Pt(),qmulti,wt2);
                        }
                    }
		...
		h_BestPhotonPt_elec1_closure->Fill(bestEMObj.Pt(),wt2);
		...
		if((EWselec && EWselec1)) {
		  ...
		  h_BestPhotonPt_EW_elec1_closure->Fill(bestEMObj.Pt(),wt2);
		...
		else
		  {
		    ...
		    h_BestPhotonPt_SP_elec1_closure->Fill(bestEMObj.Pt(),wt2);
	...
	else if(!bestEMObjIsEle)
	  {
	    ...
	    //h_SBins_v6_CD_EW_50bin_elec0->Fill(sBin6_50bin,wt);
            ...
            if(applybTagSFs && s_data.Contains("v17")){
              ...
              h2_SBins_v6_CD_EW_50bin_elec0->Fill(sBin6_50bin,qmulti,wt*prob0);
              h2_PhoPtQMultJet_elec0->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
              h2_PhoPtQMultJet_v2_elec0->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
              ...
              h_BTags_elec0->Fill(nbjets,wt*prob0);

              if(savePDFscaleUnc){
                for(int si=0;si<ScaleWeights->size();si++)
                  {
                    h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*ScaleWeights)[si]);
                    h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*ScaleWeights)[si]);
                  }
                for(int si=0;si<PDFweights->size();si++)
                  {
                    h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0*(*PDFweights)[si]);
                  }
              }
              else
                {
                  for(int si=0;si<ScaleWeights->size();si++)
                    {
                      h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
                      h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
                    }
                  for(int si=0;si<PDFweights->size();si++)
                    {
                      h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
                      h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob0);
                    }
                }
              ...
              h2_SBins_v6_CD_EW_50bin_elec0->Fill(sBin6_50bin,qmulti,wt*prob1);
              h2_PhoPtQMultJet_elec0->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
              h2_PhoPtQMultJet_v2_elec0->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
              ...
              h_BTags_elec0->Fill(nbjets,wt*prob1);
              if(savePDFscaleUnc){
                for(int si=0;si<ScaleWeights->size();si++)
                  {
                    h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*ScaleWeights)[si]);
                    h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*ScaleWeights)[si]);
                  }
                for(int si=0;si<PDFweights->size();si++)
                  {
                    h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1*(*PDFweights)[si]);
                  }
              }
              else
                {
                  for(int si=0;si<ScaleWeights->size();si++)
                    {
                      h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                      h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                    }
                  for(int si=0;si<PDFweights->size();si++)
                    {
                      h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                      h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*prob1);
                    }
		}
              ...
              if(savePDFscaleUnc){
                for(int si=0;si<ScaleWeights->size();si++)
                  {
                    h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*ScaleWeights)[si]);
                    h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*ScaleWeights)[si]);
                  }
                for(int si=0;si<PDFweights->size();si++)
                  {
                    h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*PDFweights)[si]);
                    h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt*(*PDFweights)[si]);
                  }
              }
              else
                {
                  for(int si=0;si<11;si++)
                    {
                      h2_PhoPtQMultJet_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                      h2_PhoPtQMultJet_v2_scale_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                    }
                  for(int si=0;si<112;si++)
                    {
                      h2_PhoPtQMultJet_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                      h2_PhoPtQMultJet_v2_pdf_elec0[si]->Fill(bestEMObj.Pt(),qmulti,wt);
                    }
		}


	    }
	    h_mTPhoMET_elec0->Fill(mt_pho,wt); 
	    /*               
	    h_MET_elec0->Fill(MET,wt);
	    h_METvBin2_elec0->Fill(MET,wt);
	    h_nJets_elec0->Fill(nHadJets,wt);
	    ...
	    */
            ...
            if(EWselec && EWselec1) h_hadAk8Mass_EW_elec0->Fill(Ak8Mass,wt);
            else h_hadAk8Mass_SP_elec0->Fill(Ak8Mass,wt);
	    ...
	    h_nvtx_elec0->Fill(NVtx,wt);
            h_genephoratio_elec0->Fill(ratio,wt);
	    if(photonMatchingJetIndx>=0){
	      h2_BestPhotonPt_jetmatchphoratio_elec0->Fill(bestEMObj.Pt(),(*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
	      h_jetphoratio_elec0->Fill((*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
	      if(minDRindx>=0) h2_mindr_jetmatchphoratio_elec0->Fill(minDR,(*Jets)[minDRindx].Pt()/bestEMObj.Pt(),wt);
	    }
            if(photonMatchingJetIndx<0)              
	      {
		h2_QMultlleadbJet_v2_elec0->Fill(bestEMObj.Pt(),(*Jets)[photonMatchingJetIndx].Pt()/bestEMObj.Pt(),wt);
		//                if(minDRindx>=0) h2_mindr_jetmatchphoratio_elec0->Fill(minDR,(*Jets)[minDRindx].Pt()/bestEMObj.Pt(),wt);
	    ...
	    h2_QMultlleadJetPt_elec0->Fill(bestEMObj.Pt(),qmulti,wt);
...
int Fakerate::getBinNoV7_le2(bool EWselec, bool EWselec1, bool EWselec_Htag , bool EWselec_Wtag,int nbjets ,int nHadJets){
  int sBin=-100,m_i=0;
  if(!(EWselec && EWselec1))
    {
      if(nbjets==0){
