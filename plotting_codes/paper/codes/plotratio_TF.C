...
void plotratio_TF(TString title1="full_Run2", TString ntuples="v18" , bool lowdphi=false)
  {
	...
	if(!lowdphi){
	  path="rootoutput/LL/";
	  path_le="rootoutput/LE/";
	  path_lm="rootoutput/LM/";
	  path_lmuo="rootoutput/LM_muo/";
	  path_hadtau="rootoutput/LM_hadtau/";
      }
  ...
  TString varName[1]={"AllSBins_v7_CD_SP"};
  ...
  for(int i=0; i<k;i++)
	...
	cr->Divide(total);
      ...
      TLegend *legend = new TLegend(0.35,0.75,0.89,0.89);
	...
	*/
      ...
      stack[i]->GetYaxis()->SetTitleOffset(0.92);
      ...
      //textOnTop.DrawLatexNDC(0.1,0.91,"CMS #it{#bf{}}");
      //textOnTop.DrawLatexNDC(0.12,0.91,"work in progress");
      textOnTop.DrawLatexNDC(0.1,0.92,"CMS");
      textOnTop2.DrawLatexNDC(0.157,0.92," #it{#bf{Simulation}}");

       if(title1=="2016")
    	intLumiE.DrawLatexNDC(0.8,0.92,"#bf{35.9 fb^{-1} (13 TeV)}");
      if(title1=="2017")
    	intLumiE.DrawLatexNDC(0.8,0.92,"#bf{41.5 fb^{-1} (13 TeV)}");
      if(title1=="2018")
    	intLumiE.DrawLatexNDC(0.8,0.92,"#bf{59.6 fb^{-1} (13 TeV)}");
      if(title1=="full_Run2")
	...
	TF[i]->Divide(cr);
      ...
      TF[i]->GetYaxis()->SetLabelSize(0.135);
      TF[i]->SetLineWidth(3);
      TF[i]->GetYaxis()->SetTitleOffset(0.34);
      TF[i]->GetYaxis()->SetTitleSize(0.135);
