...
const int nfiles=10,nBG=6;    //Specify no. of files
TFile *f[nfiles];
bool savePlots=1;
bool isPaper=0;
bool newbin=true;
...
//TCanvas *c_cA=new TCanvas("kinVar","plot of a kin var",1600,1000);
TCanvas *c_cA=new TCanvas("kinVar","plot of a kin var",1000,800);
...
TLegend *legend1=new TLegend(0.6,0.45,0.92,0.61);
TLegend *legend2=new TLegend(0.12,0.65,0.60,0.89);
...
void plotKinStack(TString sample="T5bbbbZg", TString varName="AllSBins_v6_CD_EW_50bin", TString ext="" ,TString year="full_Run2",int seq=0,TString path="rootoutput/nphotons/"){
//void plotKinStack(TString varName="AllSBins_v6_CD_EW_50bin", TString ext="" ,TString year="full_Run2",int seq=0,TString path="PFCaloMET_Checks/withPFCaloMET/"){
 ...
 if(varName == "MET_B"||varName == "MET_D") {xLabel = "p_{T}^{miss} (GeV)";   rebin=5; yMin=0.1,yMax = 1000000; xMin=300.0,xMax = 1500;}
  ...
  if(varName.Contains("hadAk8Mass")||varName == "hadAk8Mass_EW" ||varName == "hadAk8Mass_SP") {xLabel = "Mass_{Leading Ak8Jet} (GeV)";  rebin=25,yMin=0.1,yMax = 1000000;
  ...
  if(year=="full_Run2")
	...
	TString  fname=path+"GJetsQCD_new_v18.root";
	f[0] = new TFile(fname);
	...
	if(sample.Contains("T5bbbbZg"))
	  ...
	  {
	    fname=path+"T5qqqqHg_2250_200_FastSim_v18.root"; f[6]  = new TFile(fname);
	    fname=path+"T5qqqqHg_2250_1600_FastSim_v18.root"; f[7] = new TFile(fname);
	    fname=path+"T5qqqqHg_2250_2150_FastSim_v18.root"; f[8] = new TFile(fname);
	  }
	  ...
	  {
	    fname=path+"T6ttZg_1300_200_FastSim_v18.root"; f[6]  = new TFile(fname);
	    fname=path+"T6ttZg_1300_600_FastSim_v18.root"; f[7] = new TFile(fname);
	    fname=path+"T6ttZg_1300_1200_FastSim_v18.root"; f[8] = new TFile(fname);
	  }
	  ...
	  {
	    fname=path+"TChiNG_0_600_FastSim_v18.root"; f[6]  = new TFile(fname);
	    fname=path+"TChiNG_0_1000_FastSim_v18.root"; f[7] = new TFile(fname);
	    fname=path+"TChiNG_0_1200_FastSim_v18.root"; f[8] = new TFile(fname);
	  }
	else if(sample.Contains("EWK"))
	  {
	    col.resize(0);
	    //	    col={kCyan,kOrange,kTeal+9,kCyan-1,kGray,kRed,kBlue,kPink+10,kOrange+7,kMagenta,kBlack,kMagenta};
	    fname=path+"TChiWG_0_600_FastSim_v18.root"; f[6]  = new TFile(fname);
            fname=path+"TChiWg_0_1000_FastSim_v18.root"; f[7] = new TFile(fname);
	    fname=path+"TChiNG_0_600_FastSim_v18.root"; f[8]  = new TFile(fname);
	    fname=path+"TChiNG_0_1000_FastSim_v18.root"; f[9] = new TFile(fname);	
	  }
	else if(sample.Contains("strong"))
          {
            col.resize(0);
	    //	    legend2=new TLegend(0.12,0.67,0.60,0.89);
	    //            col={kCyan,kOrange,kTeal+9,kCyan-1,kGray,kRed,kBlue,kPink+10,kOrange+7,kMagenta,kBlack,kMagenta};
            fname=path+"T5bbbbZg_2250_200_FastSim_v18.root"; f[6]  = new TFile(fname);
            fname=path+"T5bbbbZg_2250_2100_FastSim_v18.root"; f[7] = new TFile(fname);
            fname=path+"T5ttttZg_2200_200_FastSim_v18.root"; f[8]  = new TFile(fname);
            fname=path+"T5ttttZg_2200_1975_FastSim_v18.root"; f[9] = new TFile(fname);
  ...
  for(int i=0;i<nfiles;i++){
    ...
    if(i==nBG-1) {
      ...
      //      if(xMin > -10000 && xMax < 10000) hs_var->GetXaxis()->SetRangeUser(xMin,0.1+xMax);
    }
    if(i>=nBG && i<=9){ 
      c_cA->cd(); 
    ...
    if(i==nfiles-1){ 
      hs_var->GetXaxis()->SetTitleOffset(1.12);
      hs_var->GetXaxis()->SetTitle(xLabel); hs_var->GetYaxis()->SetTitle("Events / Bin");hs_var->SetTitle(0);
      hs_var->GetYaxis()->SetTitleOffset(1.12);
  ...
  intLumiE.DrawLatexNDC(0.725,0.915,"#bf{137 fb^{-1} (13 TeV)}");
  TLatex Tl;
  Tl.SetTextSize(0.04);
  ...
  */
  ...
  if(savePlots){
    ...
    pdf="pdf/"+sample+"_"+varName_+"_full_Run2.pdf";
    png="png/"+sample+"_"+varName_+"_full_Run2.png";
    c_cA->SaveAs(pdf);
    c_cA->SaveAs(png);
...
void decorate(THStack *hs,int i,const char* fname){
  ...
  hs->GetXaxis()->SetTitleOffset(1.12);

  hs->GetXaxis()->SetTitleSize(0.05);
  hs->GetYaxis()->SetTitleSize(0.04);
  hs->GetYaxis()->SetTitleOffset(1.12);
...
void drawlegend(TH1D *hist,int i,const char* fname){
  ...
  //  else if(lName.Contains("T5bbbbZg_1600_150")){lName="T5bbbbZg 1600, 150";}
  ...
  else if(lName.Contains("T5bbbbZg_2250_1600")){lName = "pp #rightarrow #tilde{g}#tilde{g} , #tilde{g} #rightarrow b #bar{b} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 2250 GeV, m_{#tilde{#chi}_{1}^{0}} = 1600 GeV)";}
 ...
 else if(lName.Contains("T5qqqqHg_2250_200")){lName = "#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2250 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)";}
   else if(lName.Contains("T5qqqqHg_2250_1600")){lName = "#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2250 GeV, m_{#tilde{#chi}_{1}^{0}} = 1600 GeV)";}
  else if(lName.Contains("T5qqqqHg_2250_2150")){lName = "#tilde{g} #rightarrow q #bar{q} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/H #tilde{G} (m_{#tilde{g}} = 2250 GeV, m_{#tilde{#chi}_{1}^{0}} = 2150 GeV)";}
  ...
  else if(lName.Contains("T5ttttZg_2200_1000_")){lName = "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 1000 GeV)";}
    ...
    else if(lName.Contains("T5ttttZg_2200_1975_")){lName = "#tilde{g} #rightarrow t #bar{t} #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 2200 GeV, m_{#tilde{#chi}_{1}^{0}} = 1975 GeV)";}
  ...
  else if(lName.Contains("T6ttZg_1300_200")){lName = "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 200 GeV)";}
  else if(lName.Contains("T6ttZg_1300_600")){lName = "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 600 GeV)";}
  else if(lName.Contains("T6ttZg_1300_1200")){lName = "#tilde{t} #rightarrow t #tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0} #rightarrow #gamma/Z #tilde{G} (m_{#tilde{g}} = 1300 GeV, m_{#tilde{#chi}_{1}^{0}} = 1200 GeV)";}
  ...
  else if(lName.Contains("TChiWG_0_600")){lName="pp #rightarrow #tilde{#chi}_{1}^{+/-}#tilde{#chi}_{1}^{0} + #tilde{#chi}_{1}^{+/-}#tilde{#chi}_{1}^{+/-} (Wino like) ,  #tilde{#chi}_{1}^{0}/#tilde{#chi}_{1}^{+/-} #rightarrow #gamma/W^{#pm} #tilde{G} , (m_{#tilde{#chi}_{1}^{0}} = m_{#tilde{#chi}_{1}^{+/-}} = 600 GeV)";}
  ...
  else if(lName.Contains("TChiWg_0_1000")){lName="pp #rightarrow #tilde{#chi}_{1}^{+/-}#tilde{#chi}_{1}^{0} + #tilde{#chi}_{1}^{+/-}#tilde{#chi}_{1}^{+/-} (Wino like),  #tilde{#chi}_{1}^{0}/#tilde{#chi}_{1}^{+/-} #rightarrow #gamma/W^{#pm} #tilde{G}  , (m_{#tilde{#chi}_{1}^{0}} = m_{#tilde{#chi}_{1}^{+/-}} = 1000 GeV)";}
  ...
  else if(lName.Contains("TChiNG_0_600")){lName="pp #rightarrow #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1/2}^{0} + #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#pm} + #tilde{#chi}_{1}^{0}#tilde{#chi}_{2}^{0} (Higgsino like) ,  #tilde{#chi}_{1}^{0} #rightarrow #gamma/H/Z #tilde{G} , (m_{#tilde{#chi}_{1/2}^{0}}/m_{#tilde{#chi}_{1}^{#pm}} = 600 GeV)";}
  ...
  else if(lName.Contains("TChiNG_0_1000")){lName="pp #rightarrow #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1/2}^{0} + #tilde{#chi}_{1}^{#pm}#tilde{#chi}_{1}^{#pm} + #tilde{#chi}_{1}^{0}#tilde{#chi}_{2}^{0} (Higgsino like) ,  #tilde{#chi}_{1}^{0} #rightarrow #gamma/H/Z #tilde{G} , (m_{#tilde{#chi}_{1/2}^{0}}/m_{#tilde{#chi}_{1}^{#pm}} = 1000 GeV)";}
