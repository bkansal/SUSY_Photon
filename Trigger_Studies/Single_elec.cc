...
void Single_elec::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    */
	...
	if(nbjets==0) h_METvBin_nj2_nb0->Fill(MET,wt);
	else h_METvBin_nj2_nb1->Fill(MET,wt);
      }
      if((nHadJets >= 5 && nHadJets <= 6)){ h_METvBin_nj5->Fill(MET,wt);
	if(nbjets==0) h_METvBin_nj5_nb0->Fill(MET,wt);
	else h_METvBin_nj5_nb1->Fill(MET,wt);
      }
      if((nHadJets >= 7)) {
	h_METvBin_nj7->Fill(MET,wt);
	if(nbjets==0) h_METvBin_nj7_nb0->Fill(MET,wt);
	else h_METvBin_nj7_nb1->Fill(MET,wt);
      }
      h_STvBin->Fill(ST,wt);
	...
	if((nHadJets >= 2 && nHadJets <= 4)) {
	  h_METvBin_nj2_Req->Fill(MET,wt);
	  if(nbjets==0) h_METvBin_nj2_nb0_Req->Fill(MET,wt);
          else h_METvBin_nj2_nb1_Req->Fill(MET,wt);
	}
	if((nHadJets >= 5 && nHadJets <= 6)){ h_METvBin_nj5_Req->Fill(MET,wt);
          if(nbjets==0) h_METvBin_nj5_nb0_Req->Fill(MET,wt);
	  else h_METvBin_nj5_nb1_Req->Fill(MET,wt);
	}
	if((nHadJets >= 7)) {
	  h_METvBin_nj7_Req->Fill(MET,wt);
          if(nbjets==0) h_METvBin_nj7_nb0_Req->Fill(MET,wt);
	  else h_METvBin_nj7_nb1_Req->Fill(MET,wt);
	}
