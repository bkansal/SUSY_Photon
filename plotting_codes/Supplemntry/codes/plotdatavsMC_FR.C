...
void plotdatavsMC_FR(TString varName="AllSBins_v6_CD_EW_50bin_elec1", TString year="full_Run2", TString ntuple="v18", bool DatavsMC=true)
{
  TFile *f1, *f2,*f3,*f4;
  ...
  if(year == "full_Run2")
    {
      filename2= path+"Run2_METdata_CR_v18.root";
    ...
    filename2= path+"Run"+year+"_METdata_CR_v18.root";
 ...
 else if(varName == "ElectronPt" || varName == "leadElectronPt" || varName=="EMObj_Pt"){
  ...
  c1 = new TCanvas("stackhist","stackhist",1600,900);   
   cr          = (TH1D*)f1->Get(varName);
    ...
    h_3=(TH1D*)f_3->FindObjectAny(varName);
    h_4=(TH1D*)f_4->FindObjectAny(varName);
    xmin_=100,xmax_=600,xmin=100,xmax=600;
    rebin=4;
    h_0->Rebin(rebin);
    h_1->Rebin(rebin);
    h_2->Rebin(rebin);
 ...
 else if(varName=="nJets" || varName=="nJets_EW" ||varName=="nJets_SP" || varName=="Qmulti")
    {
      if( varName=="Qmulti") { varName1="Q_{multi}"; rebin=2; xmin_=0,xmax_=16,xmin=0,xmax=16;}
      else {varName1="N_{jets}"; rebin=1; xmin_=2,xmax_=12,xmin=2,xmax=12;}
  c1= new TCanvas("stackhist","stackhist",1600,900);
  ...
  */
  ...
  TLegend *legend = new TLegend(0.45,0.82,0.89,0.89);
  legend->SetNColumns(1);
  legend->SetBorderSize(0);

  if(DatavsMC)
    {
      legend2->AddEntry(sr,"Data","lp");
      //  legend->AddEntry(pred_sr,"MC","lp");
      //  legend->AddEntry(sr,"MC","lp");
      legend2->AddEntry(h_0,"t #bar{t} + #gamma","f");
      legend2->AddEntry(h_1,"W(l#nu) + #gamma","f");
      legend2->AddEntry(h_2,"t #bar{t}","f");
      legend2->AddEntry(h_3,"W(l#nu) + jets","f");
