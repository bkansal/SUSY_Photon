...
void DataVsPredFit(TString iFname, TString catName){
  ...
  sampleName = "rootoutput/T5bbbbZg_2200_200_FastSim_v18.root"; fSig[0] = new TFile(sampleName);        
  sampleName = "rootoutput/T5bbbbZg_2200_2100_FastSim_v18.root"; fSig[1] = new TFile(sampleName);	
  sampleName = "rootoutput/TChiWG_0_1000_FastSim_v18.root"; fSig[2] = new TFile(sampleName); 
  ...
  */
  ...
  legend->AddEntry(h_sum,"Total uncertainty","f");
  ...
  */
  ...
  intLumiE.DrawLatexNDC(0.8,0.92,name2);
  if(saveCanvas){
    name="png/AllSBins_v6_CD_EW_50bin_unblind_prefit.png"; c_cA->SaveAs(name);
    //    c_cA->SetCanvasSize(1600,900);
    c_cA->SetCanvasSize(2600,1600);
    if(iFname.Contains("PostFit")) {name=catName+year1+"_postFit.pdf";c_cA->SaveAs(name);}
    else if(iFname.Contains("prefit")) {name="pdf/AllSBins_v6_CD_EW_50bin_unblind_prefit.pdf"; c_cA->SaveAs(name);}
