...
void phoeff_ratio(TString fname="rootoutput/phoeff/Fastsim_T5qqqqHg_2100_v18.root",TString fname1="rootoutput/phoeff/Fastsim_T5qqqqHg_2250_v18.root",TString modelName="T5qqqqHg",TString mass="diff"){
  const Int_t NRGBs = 5;
  ...
  TFile* inputFile1 = new TFile(fname1, "READ");
  //  TFile* inputFile = new TFile("T5bbbbZg_2200_1000_maskingSRbins/fitDiagnosticsTest.root", "READ");
  TH2D *h_num[9],*h_deno[9],*h_phoeff[9],*h_num1[9],*h_deno1[9],*h_phoeff1[9];
  ...
  for(int i=0; i<n; i++){
    ...
    h_num1[i] = (TH2D*)inputFile1->FindObjectAny(v_num[i]);
    h_deno1[i] = (TH2D*)inputFile1->FindObjectAny(v_deno[i]);
    ...
    h_num1[i]->Sumw2();
    h_num1[i]->SetStats(0);
    h_num1[i]->Divide(h_deno1[i]);
    h_num[i]->Divide(h_num1[i]);
    h_num[i]->SetTitle(0);
    h_num[i]->GetZaxis()->SetRangeUser(0.8,1);
