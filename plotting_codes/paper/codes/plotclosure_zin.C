void plotclosure_Zin(TString varName,  TString year,  TString ntuples , TString dir2, bool ExpvsPred = true, bool SRvsCR=false)
{
  ...
  TCanvas *c1 = new TCanvas("stackhist","stackhist",500,800);
  //  TCanvas *c1;
  ...
  if(ExpvsPred)
    {
      png = "pdf/Zin_"+varName+"_"+year+"_SF.png";
  ...
  if(ntuples=="v18"){
      png = "pdf/Zin_"+varName+"_"+year+"_norm.png";
  ...
  if(year=="full_Run2")
    intLumiE.DrawLatexNDC(0.65,0.91,"#bf{137 fb^{-1} (13 TeV)}");
  ...
  if(varName=="AllSBins_v6_CD_EW_7bin" || varName=="AllSBins_v6_CD_EW_50bin" )
    ...
    {
      legend = new TLegend(0.55,0.75,0.9,0.9);  
  ...
  if(SRvsCR)  
    {
      ...
      h3->GetYaxis()->SetTitleOffset(0.23);
    ...
    {
      h3->GetYaxis()->SetNdivisions(5);
      h3->GetYaxis()->SetTitleOffset(0.21);
