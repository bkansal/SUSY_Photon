...
void Mulijet::EventLoop(const char *data,const char *inputFileList) {
  ...
  if(do_prediction)
    {
      cout<<" RxK file opened : "<<endl;      
      TF = TFile::Open("RK_lowMET_data.root","READ"); 
      tf=(TH1D*)TF->FindObjectAny("fr");
      TF2 = TFile::Open("Kfactor.root","READ");                                                                             
      TF3 = TFile::Open("Kfactor_nbj0.root","READ");                                                                        
      TF4 = TFile::Open("Kfactor_Wtag.root","READ");                                                                        
      TF5 = TFile::Open("Kfactor_Htag.root","READ");                            
      tf2=(TH2D*)TF2->FindObjectAny("fr");                                                                                  
      tf3=(TH2D*)TF3->FindObjectAny("fr");                                                                                  
      tf4=(TH1D*)TF4->FindObjectAny("h3");                                                                                  
      tf5=(TH1D*)TF5->FindObjectAny("h3");                                                                                  
...

  {
   ...
   if(do_prediction && !old && !applyKxR)
   {
     ...
     double parX=-1,parY=-1, R_lowMET=-1, R_highMET =-1;
     parX=MET;
     parY=nHadJets;
     if(applyRf)
       {
	 if(!(EWselec && EWselec1))
	   {
	     if(nbjets==0)
	       {
		 wt1 =wt*tf3->GetBinContent(tf3->FindBin(parX,parY));
		 //cout<<" MET = "<<parX<<", Njets = "<<nHadJets<<" : R factor = "<<tf3->GetBinContent(tf3->FindBin(parX,parY))<<endl;
	       }
	     else
	       {
		 wt1 =wt*tf2->GetBinContent(tf2->FindBin(parX,parY));
                 //cout<<" MET = "<<parX<<", Njets = "<<nHadJets<<" : R factor = "<<tf2->GetBinContent(tf2->FindBin(parX,parY))<<endl;

	       }
	   }
	 else if((EWselec && EWselec1))
	   {
	     if(EWselec_Wtag && !EWselec_Htag)     { wt1=wt*tf4->GetBinContent(tf4->FindBin(MET));}// cout<<" MET = "<<parX<<" : R factor = "<<tf4->GetBinContent(tf4->FindBin(MET))<<endl;}
	     if(EWselec_Htag && !EWselec_Wtag)      { wt1=wt*tf5->GetBinContent(tf5->FindBin(MET));}// cout<<" MET = "<<parX<<" : R factor = "<<tf5->GetBinContent(tf5->FindBin(MET))<<endl;}
	   }
	 ...
	 if(MET>(METcut+100))
	   {
	     if(!(EWselec && EWselec1))
	       {
		 if(nbjets==0)
		   wt2 =wt1*tf3->GetBinContent(tf3->FindBin(METcut+100,parY))/tf3->GetBinContent(tf3->FindBin(METcut,parY));
		 else
		   wt2 =wt1*tf2->GetBinContent(tf2->FindBin(METcut+100,parY))/tf2->GetBinContent(tf2->FindBin(METcut,parY));
	       }

	     else if((EWselec && EWselec1))
	       {
		 if(EWselec_Wtag && !EWselec_Htag)     { wt2=wt1*tf4->GetBinContent(tf4->FindBin(METcut+100))/tf4->GetBinContent(tf4->FindBin(METcut));}
		 if(EWselec_Htag && !EWselec_Wtag)      { wt2=wt1*tf5->GetBinContent(tf5->FindBin(METcut+100))/tf5->GetBinContent(tf5->FindBin(METcut));}
	       }
	   }
	 else  wt2=wt1*1;
       }
   ...
   if(do_prediction && old)
     {
       ...
       if(nbjets==0){
	 if(nHadJets==2)     { wt1=wt*tf->GetBinContent(1);}
	 else if(nHadJets==3)     { wt1=wt*tf->GetBinContent(2);}
	 else if(nHadJets==4)     { wt1=wt*tf->GetBinContent(3);}
	 else if((nHadJets==5 || nHadJets==6)){ wt1=wt*tf->GetBinContent(4);}
	 else if(nHadJets>=7)   { wt1=wt*tf->GetBinContent(5);}
       }
       else{
	 if(nHadJets>=2 && nHadJets<=4)      { wt1=wt*tf->GetBinContent(6);}
	 else if((nHadJets==5 || nHadJets==6)){ wt1=wt*tf->GetBinContent(7);}
	 else if(nHadJets>=7)   { wt1=wt*tf->GetBinContent(8);}	 
       }
   ...
   if(region_A)
	 ...
	 nbjets=0;
         wt1b =wt* tf3->GetBinContent(tf3->FindBin(METcut,nHadJets));
	 if((EWselec && EWselec1)) wt1b=wt1;
	 sBin_RF_A=getBinNoV7_le2(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets);
	 h_SBins_v7_CD_SP_A->Fill(sBin_RF_A,wt1b*prob0);
	 sBin6_50bin_A= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_50bin_AB->Fill(sBin6_50bin_A,wt1b*prob0);
	 h_SBins_v6_CD_EW_50bin_A->Fill(sBin6_50bin_A,wt1b*prob0);
	 sBin6_7bin_A= getBinNoV6_EWplusSP_SR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_7bin_B->Fill(sBin6_7bin_A,wt1b*prob0);
	 sBin6 = getBinNoV7_le(nHadJets,nbjets);
	 h_SBins_v6_CD_A->Fill(sBin6,wt1b*prob0);
	 if(!(EWselec && EWselec1)){
	   h2_nJetsMET_A->Fill(MET,nHadJets,wt*prob0); 
	   h2_nJetsMET_v2_A->Fill(MET,nHadJets,wt*prob0);	 
	 }
	 nbjets=1;
         wt1b = wt*tf2->GetBinContent(tf2->FindBin(METcut,nHadJets));
	 if((EWselec && EWselec1)) wt1b=wt1;
	 sBin_RF_A=getBinNoV7_le2(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets);
	 h_SBins_v7_CD_SP_A->Fill(sBin_RF_A,wt1b*prob1);
	 sBin6_50bin_A= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_50bin_AB->Fill(sBin6_50bin_A,wt1b*prob1);
	 h_SBins_v6_CD_EW_50bin_A->Fill(sBin6_50bin_A,wt1b*prob1);
	 sBin6_7bin_A= getBinNoV6_EWplusSP_SR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_7bin_B->Fill(sBin6_7bin_A,wt1b*prob1);
	 sBin6 = getBinNoV7_le(nHadJets,nbjets);
	 h_SBins_v6_CD_A->Fill(sBin6,wt1b*prob1);
	 if(!(EWselec && EWselec1)){
   ...
   if(region_B)
	 ...
	 nbjets=0;
	 wt1b = wt*tf3->GetBinContent(tf3->FindBin(METcut,nHadJets));
	 if(MET>METcut+100) wt1b = wt1b*tf3->GetBinContent(tf3->FindBin(METcut+100,nHadJets))/tf3->GetBinContent(tf3->FindBin(METcut,nHadJets));
	 if((EWselec && EWselec1)) wt1b=wt1;

	 sBin_RF_B=getBinNoV7_le2(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets);
	 h_SBins_v7_CD_SP_B->Fill(sBin_RF_B,wt1b*prob0);
	 sBin6_50bin_B= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_50bin_B->Fill(sBin6_50bin_B,wt1b*prob0);
	 h_SBins_v6_CD_EW_50bin_AB->Fill(sBin6_50bin_B,wt1b*prob0);
	 sBin6_7bin_B= getBinNoV6_EWplusSP_SR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
	 h_SBins_v6_CD_EW_7bin_B->Fill(sBin6_7bin_B,wt1b*prob0);
	 sBin6 = getBinNoV7_le(nHadJets,nbjets);
	 h_SBins_v6_CD_B->Fill(sBin6,wt1b*prob0);
	 if(!(EWselec && EWselec1))
	   { h2_nJetsMET_A->Fill(MET,nHadJets,wt2*prob0); h2_nJetsMET_v2_A->Fill(MET,nHadJets,wt2*prob0);}

	 nbjets=1;
	 wt1b = wt*tf2->GetBinContent(tf2->FindBin(METcut,nHadJets));
         if(MET>METcut+100) wt1b = wt1b*tf2->GetBinContent(tf2->FindBin(METcut+100,nHadJets))/tf2->GetBinContent(tf2->FindBin(METcut,nHadJets));
	 if((EWselec && EWselec1)) wt1b=wt1;

         sBin_RF_B=getBinNoV7_le2(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets);
         h_SBins_v7_CD_SP_B->Fill(sBin_RF_B,wt1b*prob1);
         sBin6_50bin_B= getBinNoV6_EWplusSP_CR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
         h_SBins_v6_CD_EW_50bin_B->Fill(sBin6_50bin_B,wt1b*prob1);
         h_SBins_v6_CD_EW_50bin_AB->Fill(sBin6_50bin_B,wt1b*prob1);
         sBin6_7bin_B= getBinNoV6_EWplusSP_SR(EWselec,EWselec1 ,EWselec_Htag,EWselec_Wtag,nHadJets,nbjets,MET);
         h_SBins_v6_CD_EW_7bin_B->Fill(sBin6_7bin_B,wt1b*prob1);
         sBin6 = getBinNoV7_le(nHadJets,nbjets);
         h_SBins_v6_CD_B->Fill(sBin6,wt1b*prob1);
         if(!(EWselec && EWselec1))
