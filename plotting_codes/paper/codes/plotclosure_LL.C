void plotclosure_LL(TString varName="AllSBins_v6_CD_EW_50bin",  TString year="full_Run2",  TString ntuples="v18" , bool ExpvsPred = true, bool SRvsCR=false)
{
  ...
  //// Systematics for LL ////////////
  TFile *f_= new TFile("rootoutput/Lostlepton_highdphi.root");
  ...
  */
  ...
  pred_sr->GetYaxis()->SetTitleOffset(0.9);
 ...
 if(varName=="AllSBins_v6_CD_EW_50bin"){
  ...
  textOnTop2.DrawLatexNDC(0.157,0.915," #it{#bf{Simulation}}");
  if(year=="2016")
  ...
  if(varName=="AllSBins_v6_CD_EW_50bin"  ){
    ...
    Tl.DrawLatex(35.25,2*yset_,"V tag");
    Tl.DrawLatex(41.25,2*yset_,"H tag");
...
Tl.DrawLatex(3,2*yset_,"N^{ 0}_{ 2-4}");
  ...
  if(varName=="AllSBins_v6_CD_SP"){
    ...
    Tl.DrawLatex(20.5,0.007,"N^{>0}_{ 2-4}");
