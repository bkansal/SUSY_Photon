void plotsys_pred(TString sys="default",TString year="full_Run2",int xmin=1,int xmax=46)
{
  // gStyle->SetOptStat(0);
    gStyle->SetPalette(1);
 ...
 if(year == "full_Run2")
   {
     ...
     filename2  = path2+"Run2_METdata_CR_v18.root";
   }
  ...
  TH1D *total;
  int rebin=1;
  TString varname0=varname+"_elec1_closure";
  TString varname1=varname+"_elec1_closure";
  gStyle->SetOptStat(0);
    {
     ...
     for(int i=0 ; i<xmax; i++)
	{
      ...
      for(int i=0 ; i<xmax; i++)
	cout<<sr1->GetBinContent(i)<<" : "<<sr2->GetBinContent(i)<<" : "<<pull2->GetBinContent(i)<<endl;
      */
