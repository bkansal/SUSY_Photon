...
void GetEfficiency(TString year,TString v1="default"){
  ...
  //  f[0] = new TFile("MET_eff/lowdphi/Run2016_SingleElectron_17Jul2018.root");
  //  f[1] = new TFile("MET_eff/lowdphi/Run2017_SingleElectron_17Jul2018.root");
  //  f[2] = new TFile("MET_eff/lowdphi/Run2018_SingleElectron_17Jul2018.root");
  if(v1=="era")
    {
  ...
  TF1 *f1=new TF1("f1","((TMath::Erf((x-[0])/[1])+1)/2)*[2]",280,1000);
  ...
  for(int i=0;i<nfiles;i++){
    ...
    h_grAE[i]->SetMaximum(1.1);
      ...
      h_grAE[i]->GetXaxis()->SetRangeUser(280,1000);
    //    h_grAE[i]->GetXaxis()->SetRangeUser(180,300);
  ...
  if(v1=="era")
    ...
    name="pdf/trgeff_MET_lowdphi.pdf";
  else
    name="pdf/trgeff_MET_lowdphi_"+year+".pdf";
  cA->SaveAs(name);
}
