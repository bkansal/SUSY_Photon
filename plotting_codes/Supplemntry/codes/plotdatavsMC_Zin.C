...
void plotdatavsMC_Zin(TString varName,  TString year,  TString ntuples , bool ExpvsPred = true, bool SRvsCR=false)
{
  ...
  path="rootoutput/withphicut_zmass60to120_data/" , elec=false, muon=false;
  if(ExpvsPred) path = "rootoutput/withphicut_zmass60to120_data_PF/" , elec=false, muon=false;  
  ...
  pred_sr->SetTitle(title);
 ...
 else if(varName=="MET" || varName=="JetPt" || varName=="METvBin2" || varName=="JetPt1" || varName=="JetPt2" || varName=="MET_EW" ||varName=="hadAk8jetPt" )
   {
    ...
    title="#||{#vec{p}_{T}(l^{+}l^{-}) + #vec{p}_{T}^{ miss}} (GeV)";
     ...
     if(varName=="METvBin2")    xmin_=200,xmax_=1000,xmin=200,xmax=1000,rebin=1;
 ...
 /*  cout<<"==============="<<endl; */
  ...
  //TPad *pad1 = new TPad("pad1","pad1",0,0.9,1,0.1);                                                                                                         
  TPad *pad2 = new TPad("pad1","pad1",0,0,1,0.35);
  pad2->SetTopMargin(0);
  pad2->SetBottomMargin(0.3);
  pad2->SetGridy(0);
  ...
  textOnTop2.DrawLatexNDC(0.17,0.915,"#it{#bf{Supplementary}}");
  if(year=="2016")
  ...
  TLegend *legend = new TLegend(0.5,0.8,0.94,0.89);
  ...
  if(ExpvsPred) legend->AddEntry(sr,"#beta#upointData","lp");

  if(!ExpvsPred){
    
    legend = new TLegend(0.45,0.75,0.895,0.895);
    legend->SetNColumns(3);
    legend->SetBorderSize(0);
    legend->SetTextSize(0.04);
    legend->AddEntry(sr,"Data","le1");
  ...
  if(varName=="AllSBins_v6_CD"){
    ...
    TLine *line1V6=new TLine( 1,ymin_-0.05,  1,ymax_-0.05);
    TLine *line2V6=new TLine( 2,ymin_-0.05,  2,ymax_-0.05);
    ...
    TArrow *arrow1 = new TArrow( 1, 2*yset_, 2, 2*yset_,0.01,"<|>");
    TArrow *arrow2 = new TArrow( 2, 2*yset_, 3, 2*yset_,0.01,"<|>");
    ...
    TLatex Tl;
    Tl.SetTextSize(0.05);
    Tl.DrawLatex(1.4,6*yset_,"N_{ b-tags} = 0");
    Tl.DrawLatex(2.4,6*yset_,"N_{ b-tags} #geq 1");
    Tl.DrawLatex(3.5,6*yset_,"N_{ bjets} > 1");
  ...
  h3->SetTitle(0);
  if(varName=="AllSBins_v6_CD_EW_50bin") h3->GetXaxis()->SetTitle("Bin index");
  ...
  if(ExpvsPred)
  h3->GetYaxis()->SetTitle("#beta#upointData/MC");
  else
   h3->GetYaxis()->SetTitle("#frac{Data}{MC}");
  TLine *line,*line1;
 ...
 if(year=="2017" || year=="2018")
  ...
  if(varName=="AllSBins_v6_CD"){
    ...
    TLine *line2V7=new TLine( 2,ymin+0.1,  2,ymax+0.1);
    ...
    h3->GetXaxis()->SetTickLength(0);
    TF1 *f2_=new TF1("f2_","x",1.5,3.5);
    TGaxis *A2 = new TGaxis(1,ymin_+0.1,3,ymin_+0.1,"f2_",2,"US");
    A2->SetTickSize(0.05);
    A2->Draw("B");
