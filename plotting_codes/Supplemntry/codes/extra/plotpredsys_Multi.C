void plotpredsys_multi()
{
  gStyle->SetOptStat(0);
  TLatex textOnTop,intLumiE,textOnTop2;

  TString path1="../multijet/";
  TString filename1  = path1+"RK_lowMET_data.root";
  TFile *f1 = new TFile(filename1);
  TCanvas *c1= new TCanvas("stackhist","stackhist",1600,900);
  c1->cd();
  TH1D *pred = (TH1D*)f1->Get("AllSBins_v6_CD_EW_50bin_elec1_closure");
  TH1D *sr = (TH1D*)f1->Get("AllSBins_v6_CD_EW_50bin_elec1_closure");
  TH1D *fr = (TH1D*)f1->Get("fr");
  TH1D *cr = (TH1D*)f1->Get("AllSBins_v6_CD_EW_50bin_elec1");
  //  pred->Print("all");
  // cr->Print("all");
  TH1D *syserr = (TH1D*)f1->Get("syserr");
  ...
  for(int i=1; i<46; i++)
    {
      predsys->SetBinError(i+1,pred->GetBinContent(i+1)*pow(pow(syserr->GetBinContent(i+1),2),0.5));
  ...
  cr->SetLineWidth(2);
  pred->SetLineColor(kCyan);
  pred->SetFillColor(kCyan);
  ...
  legend->Draw();
  c1->SaveAs("png/Multi_prediction.png");
  c1->SaveAs("pdf/Multi_prediction.pdf");
