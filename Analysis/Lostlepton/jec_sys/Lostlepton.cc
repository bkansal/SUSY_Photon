...
void Lostlepton::EventLoop(const char *data,const char *inputFileList) {
  ...
  TFile *pufile,*TF,*E_SF,*Egamma_SF,*TF2;
  ...
  if(highdphi)
  {
    //    if(elec) TF = TFile::Open("Runfull_Run2_TF_LL_v6_CD.root","READ");
    if(muon) TF = TFile::Open("Runfull_Run2_TF_LM_v6_CD.root","READ");
    if(elec) TF = TFile::Open("lostlepton_highdphi.root","READ");

  }
  else
    {
      //      if(elec) TF = TFile::Open("Runfull_Run2_TF_lowdphi_LL_v6_CD.root","READ");
      if(elec) TF = TFile::Open("Runfull_Run2_TF_LL_v6_CD.root","READ");
      if(muon) TF = TFile::Open("Runfull_Run2_TF_LM_v6_CD.root","READ");
    }
  if(!full_Run2)
...
HEMaffected = true; break;}
   ...
   if(s_data.Contains("v17_2016"))
    ...
    if(!highdphi)
      {
	//        process = process && !bestPhoHasPxlSeed && bestPhoton.Pt()>=100 && ST>300 && nHadJets>=2 && MET > 200 && NMuons==0 && isoMuonTracks==0 && isoPionTracks==0;
	process = process && !bestPhoHasPxlSeed && bestPhoton.Pt()>=100 && ST>300 && nHadJets>=2 && MET > 200 && (!(dPhi_METjet1 > 0.3 && dPhi_METjet2 > 0.3)) && NMuons==0 && isoMuonTracks==0 && isoPionTracks==0;
      }
	...
	if(!s_data.Contains("data") ){
	  ...
	  */
	  ...
	  if((NElectrons==0 && elec)){
	    ...
	    //	    if(isoElectronTracks!=0 || isoMuonTracks!=0 )continue;
	    elec_reco0++;
            total++;
	    //////////////////////////////////////////////////////////////                                                    
...
void Lostlepton::changeJets(int jec2Use, int jer2Use, int jet2Vary, bool varyMasswithJEC){
